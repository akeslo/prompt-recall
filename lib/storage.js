...
async function clearGlobalPin() {
  return new Promise((resolve, reject) => {
    chrome.storage.sync.remove(GLOBAL_PIN_KEY, () => {
      if (chrome.runtime.lastError) reject(chrome.runtime.lastError);
      else resolve();
    });
  });
  ...
  return new Promise((resolve) => {
    chrome.storage.sync.get('prompts', async (items) => {
      if (chrome.runtime.lastError) {
        console.error('Error checking for legacy prompts:', chrome.runtime.lastError);
        resolve();
        return;
      }
      if (items.prompts && Array.isArray(items.prompts) && items.prompts.length > 0) {
        try {
          ...
          await new Promise((r, j) => chrome.storage.sync.set({ prompts_meta: meta }, () => chrome.runtime.lastError ? j(chrome.runtime.lastError) : r()));
          await new Promise((r, j) => chrome.storage.local.set(contentMap, () => chrome.runtime.lastError ? j(chrome.runtime.lastError) : r()));

          // Clear legacy
          await new Promise((r, j) => chrome.storage.sync.remove('prompts', () => chrome.runtime.lastError ? j(chrome.runtime.lastError) : r()));
          console.log('Migration complete.');
        } catch (error) {
          // A failed migration must not hang checkAndMigrate forever — resolve
          // and let the caller proceed; the legacy 'prompts' key survives to
          // retry the migration on the next call.
          console.error('Error migrating legacy prompts:', error);
        }
      }
      resolve();
    });
  ...
  try {
    ...
    if (variants !== undefined) {
      if (variants.length > 0) {
        ...
        await new Promise((r, j) => chrome.storage.local.remove(`${id}_v`, () => chrome.runtime.lastError ? j(chrome.runtime.lastError) : r()));
      }
    }
...
async function deletePrompt(id) {
  try {
    ...
    // Remove from Local
    await new Promise((resolve, reject) => {
      chrome.storage.local.remove([id, `${id}_v`, `${id}_media`], () => {
        if (chrome.runtime.lastError) reject(chrome.runtime.lastError);
        else resolve();
      });
    });
...
async function importPrompts(jsonString, merge = true) {
  try {
    ...
    if (!merge) {
      ...
      await new Promise((r, j) => chrome.storage.sync.remove('prompts_meta', () => chrome.runtime.lastError ? j(chrome.runtime.lastError) : r()));
      if (localKeys.length > 0) {
        await new Promise((r, j) => chrome.storage.local.remove(localKeys, () => chrome.runtime.lastError ? j(chrome.runtime.lastError) : r()));
      }
    }
