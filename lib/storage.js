...
async function getPromptMedia(id) {
  return new Promise(resolve => {
    chrome.storage.local.get(`${id}_media`, items => {
      resolve(items[`${id}_media`] || { before: null, after: null });
    });
  });
}
...
async function savePromptMedia(id, media) {
  ...
  if (!hasContent) {
    await new Promise(r => chrome.storage.local.remove(`${id}_media`, r));
    return false;
  }
  await new Promise(r => chrome.storage.local.set({ [`${id}_media`]: media }, r));
  return true;
}
...
async function importPrompts(jsonString, merge = true) {
  try {
    ...
    for (const prompt of importedPrompts) {
      ...
      await savePrompt(prompt.title, prompt.content, prompt.tags, {}, prompt.variants || []);
    }
